...
class SaveModelCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            self.model.save(os.path.join(self.save_path + "model/", f"modelVx_{self.num_timesteps}.zip"))
        return True
...
if __name__ == "__main__":
    ...
    # 100HZ控制帧率 + 非标准化环境
    tb_log_name = "PPO-TrackVelocity-100HZ-xvel"
...
# -----------------------------加载模型检验时注释该部分--------------------------------
    ...
    model = PPO.load(log_dir + "ppo_phantomx_trackvel", env=env)
    # model = PPO.load(log_dir + "model/modelV4_13600000", env=env)
    # model = PPO.load(log_dir + "zip/ppo_phantomx_jump", env=env)

    env.set_goal_state([-0.5, 0.0, 0.0])
