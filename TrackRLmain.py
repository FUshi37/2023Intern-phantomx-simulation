...
class SaveModelCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            self.model.save(os.path.join(self.save_path + "model/", f"modelVxPSAC_{self.num_timesteps}.zip"))
        return True
...
# -----------------------------加载模型检验时注释该部分--------------------------------
    ...
    #     total_timesteps=8000*200, reset_num_timesteps=True, tb_log_name=tb_log_name, callback=callback
