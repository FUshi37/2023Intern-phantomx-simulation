...
class Phantomx:
    ...
    def change_dynamics(self):
        ...
        self._pybullet_client.changeDynamics(self.my_phantomx, 3, lateralFriction=0.5)
        self._pybullet_client.changeDynamics(self.my_phantomx, 7, lateralFriction=0.5)
        self._pybullet_client.changeDynamics(self.my_phantomx, 11, lateralFriction=0.5)
        self._pybullet_client.changeDynamics(self.my_phantomx, 15, lateralFriction=0.5)
        self._pybullet_client.changeDynamics(self.my_phantomx, 19, lateralFriction=0.5)
        self._pybullet_client.changeDynamics(self.my_phantomx, 23, lateralFriction=0.5)
    ...
    def ConvertActionToLegAngle_Tripod(self, actions):
        ...
        for i in range(18):
            if i%3 == 0:
                # joint_angle[i] = self.map_range(actions[i], -1.0, 1.0, -0.523599, 0.523599)
                joint_angle[i] = actions[i] * A1
                joint_angle[i] = -joint_angle[i]
            elif i%3 == 1:
                # joint_angle[i] = self.map_range(actions[i], -0.0, 2.0, 0.8, 0.3)
                joint_angle[i] = actions[i] * A2 + B2
            else:
                # joint_angle[i] = self.map_range(actions[i], -0.0, 2.0, 0.2, 0.0)
                # joint_angle[i] = self.map_range(actions[i], -0.0, 2.0, 0.3, 0.8)
                joint_angle[i] = actions[i] * A3 + B3
                # joint_angle[i] = 0.3
        for i in range(9):
            if (i+9)%3 == 0:
                # joint_angle[i+9] = self.map_range(actions[i+9], -1.0, 1.0, -0.523599, 0.523599)
                joint_angle[i+9] = actions[i+9] * A1
            elif (i+9)%3 == 1:
                # joint_angle[i+9] = self.map_range(actions[i+9], -0.0, 2.0, 0.8, 0.3)
                joint_angle[i+9] = actions[i+9] * A2 + B2
            else:
                # joint_angle[i+9] = self.map_range(actions[i+9], -0.0, 2.0, 0.3, 0.8)
                joint_angle[i+9] = actions[i+9] * A3 + B3
                # joint_angle[i+9] = 0.3
        return joint_angle
