...
import numpy as np
from gym.utils import seeding
from phantomx_env.envs.hexapod import Phantomx
import math
import time
...
class PhantomxGymEnv(gym.Env):
    ...
    def __init__(self, urdf_root=pybullet_data.getDataPath(), 
        ...
        # self._objective_weights = [distance_weight, energy_weight, drift_weight, shake_weight, abs_x_weight, x_ori_weight, y_ori_weight, abs_y_weight, z_ori_weight, dispersed_weight]
        self._objective_weights = [distance_weight, drift_weight, energy_weight, shake_weight, height_weight, shakevel_weight]
        # self._objective_weights = [distance_weight, drift_weight, energy_weight, shake_weight, height_weight, action_change_weight]
        self._objectives = []
        if self._is_render:
    ...
    def _reward(self, observations):        
        ...
        # current_basey_position = observations[0] * 10
        # current_base_velocity = observations[5:8] * 5
        # current_base_angvelocity = observations[8:11] * 10
        # orientation = observations[1:5]
        # current_joint_angles = observations[11:29] * 1.5
        # current_joint_angvelocities = observations[29:47] * 20
        # current_joint_torques = observations[47:65] * 50
        # current_CPG_data = observations[65:101] * 4
        current_basey_position = observations[0]
        current_base_velocity = observations[5:8]
        current_base_angvelocity = observations[8:11]
        orientation = observations[1:5]
        current_joint_angles = observations[11:29]
        current_joint_angvelocities = observations[29:47]
        current_joint_torques = observations[47:65]
        current_CPG_data = observations[65:101]
    ...
    def _termination(self):
        ...
        # return self.is_fallen() or position[2]>0.6 or distance > self._distance_limit
    ...
    def _get_observation_upper_bound(self):
        ...
        # upper_bound = np.zeros(46)
        # upper_bound[0:4] = 4.0  # base_orientation
        # upper_bound[4:7] = 10.0 #base linear vel
        # upper_bound[7:10] = 10.0 #base angular vel
        # for i in range(18):
        #     if i%3 == 0:
        #         # upper_bound[13+i] = 0.523599
        #         upper_bound[10+i] = 0.7
        #     elif i%3 == 1:
        #         # upper_bound[13+i] = 1.5708
        #         upper_bound[10+i] = 1.8
        #     else:
        #         # upper_bound[13+i] = 0.6179939
        #         upper_bound[10+i] = 0.8
        # upper_bound[27:45] = 20.0 #joint vel


        upper_bound = np.zeros(101)
        upper_bound[0] = 1.0 # base_position y
        upper_bound[1:5] = 1.0  # base_orientation
        upper_bound[5:8] = 1.0 #base linear vel
        upper_bound[8:11] = 1.0 #base angular vel
        for i in range(18):
            if i%3 == 0:
                # upper_bound[13+i] = 0.523599
                upper_bound[11+i] = 1.0#0.7
            elif i%3 == 1:
                # upper_bound[13+i] = 1.5708
                upper_bound[11+i] = 1.0#1.5
            else:
                # upper_bound[13+i] = 0.6179939
                upper_bound[11+i] = 1.0#1.5
        upper_bound[29:47] = 1.0 #joint vel
        upper_bound[47:65] = 1.0 #joint torque
        upper_bound[65:101] = 1.0 #CPG data

        return upper_bound
    
    def _get_observation_lower_bound(self):
        # lower_bound = np.zeros(46)
        # lower_bound[0:4] = -4.0
        # lower_bound[4:7] = -10.0
        # lower_bound[7:10] = -10.0
        # for i in range(18):
        #     if i%3 == 0:
        #         # lower_bound[13+i] = -0.523599
        #         lower_bound[10+i] = -0.7
        #     elif i%3 == 1:
        #         # lower_bound[13+i] = -1.5708
        #         lower_bound[10+i] = -1.8
        #     else:
        #         # lower_bound[13+i] = -1.5708
        #         lower_bound[10+i] = -0.8
        # lower_bound[27:45] = -20.0

        lower_bound = np.zeros(101)
        lower_bound[0] = -1.0
        lower_bound[1:5] = -1.0
        lower_bound[5:8] = -1.0
        lower_bound[8:11] = -1.0
        for i in range(18):
            if i%3 == 0:
                # lower_bound[13+i] = -0.523599
                lower_bound[11+i] = -1.0
            elif i%3 == 1:
                # lower_bound[13+i] = -1.5708
                lower_bound[11+i] = -1.0
            else:
                # lower_bound[13+i] = -1.5708
                lower_bound[11+i] = -1.0
        lower_bound[29:47] = -1.0
        lower_bound[47:65] = -1.0
        lower_bound[65:101] = -1.0
