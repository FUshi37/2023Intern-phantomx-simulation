...
class PhantomxGymEnv(gym.Env):
    ...
    def reset(self):
        ...
        # radius = 0.1
        # height = 0.2
        # collision_shape_id = self._pybullet_client.createCollisionShape(shapeType=self._pybullet_client.GEOM_CYLINDER,
        #                                     radius=radius,
        #                                     height=height)
        
        # # 创建可视形状 (可选)
        # visual_shape_id = self._pybullet_client.createVisualShape(shapeType=self._pybullet_client.GEOM_CYLINDER,
        #                                     radius=radius,
        #                                     length=height,
        #                                     rgbaColor=[0, 1, 0, 1])  # 绿色

        # # 创建多体，指定碰撞形状和可视形状
        # cylinder_id = self._pybullet_client.createMultiBody(baseMass=1.0,
    ...
    def _reward_veltrack(self, observations, action):
        ...
        current_base_velocity = observations[11:14] * 100#4
        current_base_angvelocity = observations[14:17] * 100#30
    ...
    def get_direction_reward(self, aim_yaw, current_orientation):
        _, _, yaw = quaternion_to_euler(current_orientation)
        # aim_yaw = np.arctan2(current_goal_state[1], current_goal_state[0])
        yaw_penalty = self.penalty_function(aim_yaw, yaw, False)
        if self._env_step_counter == 0:
