...
class OnlinePhantomxCPG(object):
    ...
    def __init__(self, leg_num = 6, motor_num = 3, coupling_factor = 2, 
        ...
        if self._coupling == True:
            self._body_oscillator = [self.init_oscillator(omega_sw=1*np.pi)] * self._leg_num #6个hip joint
            self._limb_oscillator = [self.init_oscillator(mu=4, omega_sw=1*np.pi)] * (self._leg_num - 1) * self.__motor_num #12个limb joint\
