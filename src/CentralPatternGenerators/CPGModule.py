...
class PhantomxCPG(object):
    ...
    def __init__(self, leg_num = 6, motor_num = 3, coupling_factor = 0.35, 
        ...
        if self._coupling == True:
            ...
            self._limb_oscillator = [self.init_oscillator(omega_sw=2*np.pi)] * (self._leg_num - 1) * self.__motor_num #12个limb joint\
    ...
    def coupling_hopf(self, pos, time_step):
        ...
        lx = [x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, y18]
    ...
    def TripodGait(self, leg_id, t, data):
        ...
        for i in range(len(data[:, leg_id*2-1])):
            delta_data = 0
            if i != 0:
                delta_data = data[i, leg_id*2-1] - data[i - 1, leg_id*2-1]
            if delta_data < 0:
                data[i, self._leg_num*2 + leg_id*4-3] = -1
                data[i, self._leg_num*2 + leg_id*4-1] = -1
        plt.plot(t[9000:10000], data[:, leg_id*2-1][9000:10000], label="hip-y")
        plt.plot(t[9000:10000], data[:, self._leg_num*2 + leg_id*4-3][9000:10000], label="knee-y")
        plt.plot(t[9000:10000], data[:, self._leg_num*2 + leg_id*4-1][9000:10000], label="ankle-y")
        plt.legend()
        plt.show()
        return t[i]
...
if __name__ == "__main__":
    ...
    BodyCPGMOdule.body_show(data[8000:10000], t[8000:10000])
    BodyCPGMOdule.limb_show(data[9000:10000], t[9000:10000], 2)
